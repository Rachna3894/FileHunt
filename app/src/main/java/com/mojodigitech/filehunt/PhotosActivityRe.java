...
import static com.mojodigitech.filehunt.Class.Constants.DELETED_IMG_FILES;
import static com.mojodigitech.filehunt.Utils.Utility.pathToBitmap;
...
import com.mojodigitech.filehunt.Adapter.MultiSelectAdapter;
import com.mojodigitech.filehunt.AddsUtility.SharedPreferenceUtil;
import com.mojodigitech.filehunt.AsyncTasks.encryptAsyncTask;
...
public class PhotosActivityRe extends AppCompatActivity implements AlertDialogHelper.AlertDialogListener ,MultiSelectAdapter.ImgListener, encryptAsyncTask.EncryptListener/*, PurchasesUpdatedListener*/ {
       ...
       /* if(AddConstants.checkIsOnline(mcontext) && adContainer !=null && mPrefs !=null)
        {
            ...
            {
                adutil.dispFacebookBannerAdd(mcontext, mPrefs, PhotosActivityRe.this);
            }
        }
        else {
                ...
                case R.id.action_hide:
                    if(Utility.isManualPasswordSet()) {
                        ...
                        if (multiselect_list.size() >= 1) {
                            ...
                            /*if(Utility.getHiddenFileLimit()>=5 && !queryPrefPurchases())
                            {
                                 dispPaymentReminderDialog(mcontext);
...
//        startActivity(intent);
   ...
   /* public void makeConnectioBillingClient()
    {
        *//*in app billing code*//*
        ...
        mBillingClient = BillingClient.newBuilder(PhotosActivityRe.this).setListener(this).enablePendingPurchases().build();
        mBillingClient.startConnection(new BillingClientStateListener() {
            ...
            public void onBillingServiceDisconnected() {
                ...
                Toast.makeText(PhotosActivityRe.this,  "billing connection failure", Toast.LENGTH_SHORT);
    ...
    *//*google in app billig code functions*//*
    ...
    public void onPurchasesUpdated(BillingResult responseCode, @Nullable List<Purchase> purchases) {
        ...
        if (responseCode.getResponseCode() == BillingClient.BillingResponseCode.OK
            ...
            mPrefs.setBoolanValue(Constants.pref_subscribe,true);
            //setAdFree(true);
    ...
    public  Boolean queryPrefPurchases() {
        Boolean adFree = mPrefs.getBoolanValue(Constants.pref_subscribe, false);
    ...
    *//*is being called on maina activity on create function*//*
        ...
        if (purchasesResult != null) {
            ...
            if (!purchasesList.isEmpty()) {
                for (Purchase purchase : purchasesList) {
                    if (purchase.getSkus().equals(Constants.ITEM_SKU_SUBSCIBE)) {
                        mPrefs.setBoolanValue(Constants.pref_subscribe,true);
                    }
                }
            }
    ...
    *//*billing code part *//*
    ...
    public  void  dispPaymentReminderDialog(final Context mContext)
    {
        // payment

        final Dialog dialog = new Dialog(mContext);
        dialog.setContentView(R.layout.dialog_payment_reminder);

        TextView subscibeText=dialog.findViewById(R.id.subscibeText);


        TextView view_save = dialog.findViewById(R.id.View_subscibe);
        TextView view_cancel = dialog.findViewById(R.id.View_cancel);

        subscibeText.setTypeface(Utility.typeFace_adobe_caslonpro_Regular(mContext));
        view_save.setTypeface(Utility.typeFace_adobe_caslonpro_Regular(mContext));
